...
describe('retrieve all customers', async () => {
  let app: Express
  beforeAll(async () => {
  ...
  it('retrieve success', async () => {
    ...
    await customerFactory.createMany(3)
    ...
    expect(response.body.data[0].name).toStrictEqual(customers.data[0]['name'])
    expect(isValid(new Date(response.body.data[0].created_at))).toBeTruthy()
    expect(response.body.data[1].name).toStrictEqual(customers.data[1]['name'])
    expect(response.body.data[2].name).toStrictEqual(customers.data[2]['name'])
  ...
  it('sort data in ascending order', async () => {
    ...
    customerFactory.sequence(data)
    await customerFactory.createMany(3)

    const response = await request(app).get(`/v1/customers`).query({
      sort: 'name',
    })
  ...
  it('navigate pagination', async () => {
    ...
    expect(response.body.data[0].name).toStrictEqual(customers.data[2]['name'])
