...
export const deleteCustomerController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    const response = await DeleteCustomerUseCase.handle(
      { _id: controllerInput.httpRequest['params'].id },
      { schemaValidation, deleteCustomerRepository },
    )
