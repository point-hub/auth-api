...
export const updateCustomerController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    const response = await UpdateCustomerUseCase.handle(
      {
        _id: controllerInput.httpRequest['params'].id,
        data: controllerInput.httpRequest['body'],
      },
      { schemaValidation, updateCustomerRepository, uniqueValidation, objClean },
    )
