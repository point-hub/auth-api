...
export const updateManyClientController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    const response = await UpdateManyClientUseCase.handle(
      {
        filter: controllerInput.httpRequest['body'].filter,
        data: controllerInput.httpRequest['body'].data,
      },
      { updateManyClientRepository, schemaValidation, objClean },
    )
