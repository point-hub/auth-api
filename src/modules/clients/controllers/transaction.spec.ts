...
describe('client use transaction', async () => {
  let app: Express
  beforeAll(async () => {
  ...
  it('transaction aborted when create failed', async () => {
    ...
    await request(app).post('/v1/clients/transaction').send(data)
  ...
  it('transaction aborted when update failed', async () => {
    ...
    // expect recorded data
    const clientRecords = await DatabaseTestUtil.retrieveAll('clients')
    expect(clientRecords.data.length).toStrictEqual(3)
  })
  it('transaction aborted when update many failed', async () => {
    ...
    // expect recorded data
    const clientRecords = await DatabaseTestUtil.retrieveAll('clients')
    expect(clientRecords.data.length).toStrictEqual(4)
    expect(clientRecords.data[0]['name']).toStrictEqual('John 5')
  })
