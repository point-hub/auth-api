...
import { type IRetrieveCustomerOutput } from './retrieve.repository'
...
export interface ICustomerQueryFilter {
...
export class RetrieveAllCustomerRepository implements IRetrieveAllCustomerRepository {
  ...
  async handle(query: IQuery): Promise<IRetrieveAllCustomerOutput> {
    const parsedQuery = QueryString.parse(query as Record<string, string>) as ICustomerQuery
  ...
  private buildPipeline(query: ICustomerQuery): IPipeline[] {
