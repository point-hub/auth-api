...
describe('retrieve all clients', async () => {
  let app: Express
  beforeAll(async () => {
  ...
  it('retrieve success', async () => {
    ...
    expect(response.body.data[0].name).toStrictEqual(clients.data[0]['name'])
    expect(isValid(new Date(response.body.data[0].created_date))).toBeTruthy()
    expect(response.body.data[1].name).toStrictEqual(clients.data[1]['name'])
    expect(response.body.data[2].name).toStrictEqual(clients.data[2]['name'])
  ...
  it('sort data in ascending order', async () => {
    ...
    clientFactory.sequence(data)
    await clientFactory.createMany(3)

    const response = await request(app).get(`/v1/clients`).query({
      sort: 'name',
    })
  ...
  it('navigate pagination', async () => {
    ...
    expect(response.body.data[0].name).toStrictEqual(clients.data[2]['name'])
