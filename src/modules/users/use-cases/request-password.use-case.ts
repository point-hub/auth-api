...
export class RequestPasswordUseCase {
  static async handle(input: IInput, deps: IDeps): Promise<void> {
    ...
    const compiledTemplate = await deps.renderHbsTemplate('modules/users/emails/request-password.hbs', {
      name: users.data[0].name,
      link: link,
    })
    await deps.sendMail(compiledTemplate, userEntity.data.email as string, 'Request reset password')
  }
}
